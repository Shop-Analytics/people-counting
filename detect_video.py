...
flags.DEFINE_string('video', './data/video/Video_2.mp4', 'path to input video or set to 0 for webcam')
...
def main(_argv):
    ...
    while True:
        ...
        #file = open("person counting.txt", "a+")
        for key, value in counted_classes.items():
            print("Number of {}s: {}".format(key, value))
            #file.write("total persons:".format(key, value))
        image = utils.draw_bbox(frame, pred_bbox, FLAGS.info, counted_classes, allowed_classes=allowed_classes,
                                read_plate=FLAGS.plate)
        #for person in counted_classes.items():
            #file.write("total person%f%d%s%n".format(person))
            #file.write(datetime.now() + timedelta(days=1))
        #file.close()
